...
## A propos
Cet outil upload les fichiers images JPG issues d'une strucutre de sauvegarde IPCAM (recu par FTP par exemple)
exemple structure de repertoire IPCAM:
```
<repertoireRacine>/
    |-/20230310/
        |-/images/*.jpg
...
## Installation des librairies nécessaires (dans environnment virtuel)
```
...
_gdrive.folder_destination_id_ -> l'id google drive du repertoire de destination

_folder.source_ -> le repertoire local contenant la structure IPCAM

Authentification oauth: suivre documentation fournie par Google pour la génération de token
https://developers.google.com/drive/api/quickstart/python?hl=fr

## Packaging
Créer une archive contenant les scripts python pour distribution
